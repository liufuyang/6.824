...
func GetTask(id string) GetTaskReply {
	args := GetTaskArgs{}
	reply := GetTaskReply{}
	call(id, "Coordinator.GetTask", &args, &reply)
	fmt.Printf("Worker %v - TYPE-%v - [X: %v Y: %v] - NReduce: %v - map file name: %v - reduce file len: %v -  %v \n",
		id, reply.TaskType, reply.FileNumberX, reply.FileNumberY, reply.NReduce, reply.InputFile, len(reply.ReduceFiles), reply.ReduceFiles)
...
func FinishTask(id string, taskType TaskType, X int, Y int, reduceFiles []string) FinishTaskReply {
	...
	call(id, "Coordinator.FinishTask", &args, &reply)
...
func CallExample() {
	...
	ok := call("expId", "Coordinator.Example", &args, &reply)
	if ok {
